...
def train_generator_PG(gen, gen_opt, bi_classify, train_samples, dis, num_batches):
    """
    ...
    for batch in range(num_batches):
        ...
        loss_a = gen.batchLoss_A(inp, target, attack_item=8887)
        # loss_b = 0
        # dataset = ClaDataset(MAX_SEQ_LEN, train_samples, s, mask_id=12102)
        ...
        loss = c*loss_c + a*loss_a
        loss.backward()
        gen_opt.step()

        print("training PG total_loss = %.4f, loss_a = %.4f, loss_b = None, loss_c = %.4f" % (loss,loss_a,loss_c))
        return loss, loss_a, loss_c
        # print("training PG total_loss = %.4f, loss_a = %.4f, loss_b = %.4f, loss_c = %.4f" % (loss,loss_a,loss_b,loss_c))
        # return loss, loss_a, loss_b, loss_c
        #print("training PG total_loss = %.4f" % (loss))
...
if __name__ == '__main__':
    ...
    for epoch in range(ADV_TRAIN_EPOCHS):
        ...
        sys.stdout.flush()
        # loss, loss_a, loss_b, loss_c = train_generator_PG(gen, gen_optimizer, bi_classify, train_samples, dis, 1)
        loss, loss_a, loss_c = train_generator_PG(gen, gen_optimizer, bi_classify, train_samples, dis, 1)
